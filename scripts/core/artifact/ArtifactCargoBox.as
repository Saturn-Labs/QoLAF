package core.artifact {
	...
	public class ArtifactCargoBox extends Sprite {
		...
		private function addToolTip() : void {
			...
			var _local4:String = a.name + "<br>";
			if(a.revealed && a.isRestricted) {
				_local4 += Localize.t("<font color=\'#ff0000\'>Requires level [level]</font>").replace("[level]",a.requiredPlayerLevel) + "<br>";
			}
			_local4 += Localize.t("Level [potential], strength [level]").replace("[level]",a.level).replace("[potential]",a.levelPotential) + "<br>";
			if(a.upgraded >= 10) {
				_local4 += Localize.t("Max Upgraded") + "<br>";
			} else if(a.upgraded > 0) {
				_local4 += Localize.t("[nr] upgrades").replace("[nr]",a.upgraded) + "<br>";
			}
			if(a.upgrading) {
				_local4 += Localize.t("Upgrading") + ": " + Util.getFormattedTime(a.upgradeTime - g.time) + "<br>";
			}
			for each(var _local1 in a.stats) {
				_local4 += ArtifactStat.parseTextFromStatType(_local1.type,_local1.value,_local1.isUnique) + "<br>";
			}
			toolTip.text = _local4;
