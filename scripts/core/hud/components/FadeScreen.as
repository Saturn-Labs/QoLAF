package core.hud.components {
	...
	public class FadeScreen extends EventDispatcher {
		...
		public function FadeScreen(stage:Stage) {
			...
			var _local2:Texture = textureManager.getTextureGUIByTextureName("loading.png");
			loadingImage = new Image(_local2);
			loadingImage.pivotX = _local2.width * 0.5;
			loadingImage.pivotY = _local2.height * 0.5;
			screen.addChild(loadingImage);
			loadingImage2 = new Image(_local2);
			loadingImage2.pivotX = _local2.width * 0.5;
			loadingImage2.pivotY = _local2.height * 0.5;
		...
		private function createPromotionButton() : void {
			...
			promotionButton = new NativeImageButton(function():void {
				var _local1:URLRequest = new URLRequest(promotionUrl);
		...
		private function createCustomPromotionButton(imgUrl:String) : void {
			var url:URLRequest;
			var img:Loader;
			if(imgUrl.length < 1) {
				return;
			...
			img.contentLoaderInfo.addEventListener("complete",function(param1:flash.events.Event):void {
				...
				if(promotionButton.width > 5 * 60) {
					promotionButton.width = 5 * 60;
				}
				if(promotionButton.height > 5 * 60) {
					promotionButton.height = 5 * 60;
				}
				promotionButton.x = stage.stageWidth - promotionButton.width;
				promotionButton.y = stage.stageHeight - promotionButton.height;
				if(promotionUrl.length > 0) {
		...
		private function onFadeOutTimerUpdate(e:TimerEvent) : void {
			screen.alpha = (fadeOutSteps - currentStep) / fadeOutSteps;
			currentStep++;
		}
		
		public function showHighscore(m:Message) : void {
			var _local2:Array = textsPvp[Math.floor(Math.random() * (texts.length - 0.1))];
			screenText.start(_local2);
			topPvpPlayersList = new TopPvPPlayersList();
			topPvpPlayersList.showHighscore(m);
		...
		private function onFadeInTimerUpdate(e:TimerEvent) : void {
		...
		public function repositionScreen(container:DisplayObjectContainer) : void {
