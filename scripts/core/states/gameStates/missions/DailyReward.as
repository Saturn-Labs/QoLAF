	...
	public class DailyReward extends Sprite {
		...
		public function draw() : void {
			...
			for(key in daily.reward) {
				rewardKey = daily.reward[key];
				reward = DataLocator.getService().loadKey("DailyMissionRewards",rewardKey);
				quantityText = new TextField(2 * 60,18,"",new TextFormat("font13",13,0xaaaaaa,"left"));
				...
				boostText = new TextField(2 * 60,18,"",new TextFormat("font13",13,0xaaaaaa,"left"));
		...
		public function getRewardAmount(reward:Object) : int {
			var _local2:int = 0;
			if(reward.key == "artifact") {
				return 1;
			}
			return int(reward.k1 + reward.k2 * g.me.level);
		}
	}
}
